...
export const departmentService = {
   ...
   */
  async list(page = 1, pageSize = 20): Promise<PaginatedResponse<DepartmentSummary>> {
   ...
   * Update a department
   */
  async update(id: string, data: DepartmentUpdate): Promise<DepartmentResponse> {
...
export const positionService = {
   ...
   * List all positions
   */
  async list(page = 1, pageSize = 20): Promise<PaginatedResponse<PositionSummary>> {
...
export const employeeService = {
  ...
  async search(query: string, limit = 20): Promise<Array<EmployeeSummary>> {
    return apiClient.get<Array<EmployeeSummary>>('/employees/search', { q: query, limit });
  },
  ...
  async getDirectReports(id: string): Promise<Array<EmployeeSummary>> {
    return apiClient.get<Array<EmployeeSummary>>(`/employees/${id}/direct-reports`);
  },
